...
public class RobotContainer {
    ...
    private static final Boolean simFuel = true;
    private static int simFuelCount = 0;
    ...
    // public static final CommandXboxController testController = new
    ...
    public RobotContainer() {
        ...
        // fuel sim setup
        if (Robot.isSimulation() && simFuel) {
            fuelSim.spawnStartingFuel(); // spawns fuel in the depots and neutral zone
            // Register a robot for collision with fuel
            fuelSim.registerRobot(
                    RobotConstants.ROBOT_WIDTH.in(Meters), // from left to right in meters
                    RobotConstants.ROBOT_LENGTH.in(Meters), // from front to back in meters
                    RobotConstants.BUMPER_HEIGHT.in(Meters), // from floor to top of bumpers in meters
                    () -> m_drivetrain.state.Pose, // Supplier<Pose2d> of robot pose
                    () -> m_drivetrain.state.Speeds); // Supplier<ChassisSpeeds> of field-centric chassis speeds

            // Register an intake to remove fuel from the field as a rectangular bounding
            // box
            fuelSim.registerIntake(
                    Meters.of(-0.3), Meters.of(0.3), 
                    Meters.of(0.3), Meters.of(0.45), // robot-centric coordinates for bounding box in meters
                    () -> m_intakePivot.isIntakeDeployed(), // (optional) BooleanSupplier for whether the intake should
                                                            // be active at a given moment
                    this::increaseSimFuel); // (optional) Runnable called whenever a fuel is intaked
    ...
    private void configureBindings() {
        ...
        if (simFuel)
            joystick.rightTrigger().whileTrue(Commands.run(()->simLaunchFuel()));
    ...
    private void increaseSimFuel() {
        ++simFuelCount;
    }

    public void simLaunchFuel() {
        if (simFuelCount == 0) return;
        simFuelCount--;

        fuelSim.launchFuel(
                MetersPerSecond.of(m_flywheel.getVelocity().in(RotationsPerSecond) * (2 * Math.PI * FlywheelConstants.kDiameter)),
                m_hood.getSetpoint().get(),
                m_turret.getAngle(),
                Inches.of(17.5));
}
}
